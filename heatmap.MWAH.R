...
pal <- colorRampPalette( c( brewer.pal( 9, 'YlOrRd' ), 'black'), bias=1 )(9)
...
p <- p + ylab("")
id.names <- scan("id.txt",text=TRUE,what='raw', quiet=TRUE)
for (i in c(1:length(id.names))){
    print (id.names[i])
    p <- p + geom_text(x=(chrom.borders[i]+chrom.borders[i+1])/2,y=-10,label=id.names[i],size=1.2)
}
...
if ( shuffle == 0 ) {
    ...
    p <- p + geom_vline( xintercept = chrom.borders, col='black', linetype='dashed', size=.2 )
    p <- p + geom_hline( yintercept = chrom.borders, col='black', linetype='dashed', size=.2 )
}
...
p <- p + theme( legend.text=element_text(size=20), legend.key.size=unit(2,"cm"), legend.key.width=unit(1.5,"cm") )
